...
### v1 (1K RPM) — Celery + Redis

```txt
Client
  ↓
POST /api/v1/eval/ragas/single-turn/async
  ↓
create_job() → Redis (status=pending, TTL=1h)
  ↓
ragas_single_turn_task.delay() → Celery queue (Redis broker)
  ↓
return {job_id}

Celery worker (evaluation queue):
  ↓
dequeue task
  ↓
update_job(status=running) → Redis
  ↓
EvaluationService.evaluate_ragas_single_turn()
  ↓
update_job(status=completed, result=...) → Redis

Client polls:
GET /api/v1/jobs/{job_id} → Redis → {status, result}
```

### v2 (100K RPM) — Kafka + worker pools

```txt
Client
  ↓
POST /api/v2/eval/ragas/single-turn
  ↓
create_job() → Redis (status=pending)
  ↓
KafkaPublisher.publish(topic=eval.requested, payload={job_id, eval_input})
  ↓
return {job_id}

eval_worker pod (Kafka consumer group eval-workers):
  ↓
consume eval.requested  ← Kafka assigns partition to pod
  ↓
update_job(status=running) → Redis
  ↓
EvaluationService.evaluate_ragas_single_turn()
...
update_job(status=completed, result=...) → Redis
  ↓
publish eval.completed → Kafka  ← downstream consumers

Client polls:
GET /api/v2/jobs/{job_id} → Redis → {status, result}
(production v2: replace Redis job store with async_jobs Postgres table)
```
...
POST /api/eval/classification
POST /api/eval/ragas/single-turn
POST /api/eval/ragas/multi-turn
```
...
POST /api/v1/eval/classification
POST /api/v1/eval/ragas/single-turn          ← sync, blocks until result
POST /api/v1/eval/ragas/single-turn/async    ← Celery dispatch → {job_id}
POST /api/v1/eval/ragas/multi-turn           ← sync
POST /api/v1/eval/ragas/multi-turn/async     ← Celery dispatch → {job_id}

GET  /api/v1/jobs/{job_id}                   ← poll Redis for status/result

POST /api/v1/workflows
GET  /api/v1/workflows
GET  /api/v1/workflows/{workflow_id}
POST /api/v1/workflows/{workflow_id}/validate
POST /api/v1/workflows/{workflow_id}/dry-run
...
GET  /api/v1/workflows/{workflow_id}/export.yaml
...
GET  /api/v1/node-configs
POST /api/v1/node-configs/seed
POST /api/v1/node-configs/validate
...
### v2 — split services + Kafka + worker pools (100K RPM)

```txt
POST /api/v2/eval/ragas/single-turn    ← Kafka publish → {job_id} (non-blocking)
POST /api/v2/eval/ragas/multi-turn     ← Kafka publish → {job_id} (non-blocking)

GET  /api/v2/jobs/{job_id}             ← poll Redis (→ Postgres in production)

POST /api/v2/evaluations/batch         ← fan-out N eval events to Kafka
GET  /api/v2/evaluations/{run_id}
GET  /api/v2/evaluations/{run_id}/report
```
...
## 34. API Versioning Strategy (v1 vs v2)

Both versions share the same domain layer (EvaluationService, RagasAdapter).
The difference is entirely in the infra and API tiers.

### Tier comparison

```txt
Concern            v1 (1K RPM)                v2 (100K RPM)
─────────────────  ─────────────────────────  ────────────────────────────────
API prefix         /api/v1/*                  /api/v2/*
Async dispatch     Celery task (Redis queue)  Kafka topic publish
Worker type        Celery worker process       Standalone Kafka consumer pod
Job result store   Redis (1h TTL)             Redis now → Postgres in prod
Worker scaling     Celery concurrency         Kafka partitions + pod replicas
Event replay       No                         Yes (Kafka retention)
Sync eval          Yes (/ragas/single-turn)   No (all async)
Infra deps         Redis                      Redis + Kafka/Redpanda
```
...
app/
├── schemas/
│   └── job.py                         ← AsyncJobResponse, JobStatusResponse (shared v1/v2)
│
├── infra/
│   ├── cache/
│   │   └── redis_client.py            ← aioredis connection pool (init/close in lifespan)
│   ├── jobs/
│   │   └── redis_job_store.py         ← create/get/update jobs in Redis
│   └── event_bus/
│       ├── events.py                  ← EvalRequestedEvent, EvalCompletedEvent, EvalFailedEvent
│       ├── publisher.py               ← EventPublisher Protocol
│       ├── redis_streams.py           ← Redis Streams publisher (v1 side events)
│       └── kafka.py                   ← aiokafka producer (v2 Kafka publisher)
│
├── workers/
│   ├── celery_app.py                  ← Celery app (broker=Redis, queues: evaluation/embedding/…)
│   ├── tasks/
│   │   └── eval_tasks.py              ← ragas_single_turn_task, ragas_multi_turn_task
│   └── eval_worker.py                 ← v2 standalone Kafka consumer (run: python -m app.workers.eval_worker)
│
└── api/
    ├── v1/
    │   ├── eval_router.py             ← sync + /async Celery-dispatch endpoints
    │   └── jobs_router.py             ← GET /v1/jobs/{job_id} (Redis)
    └── v2/
        ├── eval_router.py             ← Kafka-publish endpoints (returns job_id immediately)
        └── jobs_router.py             ← GET /v2/jobs/{job_id} (Redis → Postgres)
```
...
REDIS_PORT=6379
REDIS_SSL=false           # true in prod

# v2 (Kafka, opt-in)
KAFKA_ENABLED=true
KAFKA_BOOTSTRAP_SERVERS=localhost:9092
KAFKA_TOPIC_EVAL_REQUESTED=eval.requested
KAFKA_TOPIC_EVAL_COMPLETED=eval.completed
KAFKA_TOPIC_EVAL_FAILED=eval.failed
KAFKA_CONSUMER_GROUP_ID=eval-workers
```

### Running workers

```bash
# v1: start Celery evaluation worker
celery -A app.workers.celery_app worker -Q evaluation -l info

# v2: start standalone Kafka consumer (scale by running N copies)
...
```txt
v1 concurrency = number of Celery worker threads per pod
v2 throughput  = min(Kafka partitions on eval.requested, number of eval_worker pods)

For 100K RPM with ~200ms eval latency:
  100,000 / 60 ≈ 1,667 RPS
  1,667 × 0.2s = ~333 concurrent evals
  → 333 eval_worker pods, each on a dedicated partition
  → Set topic eval.requested to ≥ 333 partitions
```

### Upgrade path (v1 → v2)

```txt
1. Deploy Kafka/Redpanda alongside Redis.
2. Set KAFKA_ENABLED=true + KAFKA_BOOTSTRAP_SERVERS.
3. Deploy eval_worker pods (one per partition minimum).
4. Route new traffic to /api/v2/* endpoints.
5. Replace redis_job_store in v2 jobs_router with Postgres async_jobs table.
6. Keep /api/v1/* alive until all clients migrate.
```
